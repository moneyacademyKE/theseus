(ns bb-agent.goal.status
  "Phase 3 -- live status view.

  Gives a human a quick read of what the goal runner is doing or did, straight from
  ...
  (:require [clojure.edn :as edn]
            ...
            [bb-agent.goal.taxonomy :as taxonomy]
  ...
  (let [halted?     (some? bundle)
        ...
        recent      (when (seq iters)
                      (->> (rseq iters)
                           ...
                           (str/join "\n")))
        lines (cond-> []
                 true            (conj (str "─── Goal Runner Status ───"))
...
(defn summarize!
  "Read The goal runner.s structured logs for `cfg` and read and return a formatted
