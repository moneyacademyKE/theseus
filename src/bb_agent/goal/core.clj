...
(defn- halt-result!
  ...
  (let [workdir    (:workdir cfg ".")
        log-dir    (:log-dir cfg)
        tag-prefix (get-in cfg [:checkpoint :tag-prefix] "goal")
...
(defn run!
  ...
  ([cfg opts]
   (let [workdir     (:workdir cfg ".")
         lock-path   (:lock cfg (str workdir "/.bb-agent.goal.lock"))
         log-dir     (:log-dir cfg)
       ...
       (loop [iteration  0
         ...
         (let [[cfg* maybe-mtime] (if (and (:hot-reload cfg false) config-path)
           ...
           (case (:type action)
             ...
             :rollback
             (let [tag      (str (get-in cfg [:checkpoint :tag-prefix] "goal") "-last-good")
                   restored (git/rollback! workdir tag)]
             ...
             (let [attempt    (:attempt state 0)
               ...
               (if identical?
                 ...
                 (let [_        (git/tag! workdir (str (get-in cfg [:checkpoint :tag-prefix] "goal") "-last-good"))
