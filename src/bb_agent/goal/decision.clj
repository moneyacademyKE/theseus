(ns bb-agent.goal.decision
  "Pure decision-making logic for the goal-runner run loop.
  Determines the next action based on world state, configurations, and loop budgets."
  (:require [bb-agent.goal.predicates :as predicates]
...
(defn decide
  ...
  Returns:
    {:type :done}                                goal met
    {:type :halt :reason :integrity ...}         integrity violated (halt NOW)
