...
(defn transcribe
  "Attempt to transcribe one persisted audio attachment. Returns
   ...
   :reason :no-stt-tool | :failed (plus :detail on failure). Text is
   bounded by :max-chars. `exec` and `resolve-bin` are injectable for
   deterministic tests."
  [item {:keys [resolve-bin exec max-chars timeout-ms]
...
(defn annotation
  ...
  ([telegram-cfg item opts]
   (let [item (assoc item :stt-bin (:stt-bin telegram-cfg))
