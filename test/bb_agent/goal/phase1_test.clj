  ...
  (testing "stall triggers rollback to last-good; retry recovers and fulfills the goal"
    (let [dir (str (fs/create-temp-dir))
      ...
      (try
        ...
        (shq dir "git" "init" "-q")
        (shq dir "git" "config" "user.email" "goal@test")
        (shq dir "git" "config" "user.name" "goal-test")
        (spit (str dir "/.goal-marker") "GOOD\n")
        ...
        (let [cfg {:name "phase1-rollback-test"
                   ...
                   :observers {:level-count {:sh "ls level_*.txt 2>/dev/null | wc -l | tr -d ' '" :parse :int}
                               :build-ok {:sh "test -f .goal-marker && echo pass || echo fail" :parse :string}}
...
(defn -main
  ...
  [& _]
  (let [summary (clojure.test/run-tests 'bb-agent.goal.phase1-test)]
