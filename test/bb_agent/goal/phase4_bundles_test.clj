(ns bb-agent.goal.phase4-bundles-test
  "Phase 4 -- composable goal bundles + the example config library.
...
(deftest expand-bundles-accepts-a-vector-of-bundles
  (testing "a vector of bundle keywords is flattened in order"
    (let [ax (config/expand-bundles {:integrity-bundle [:gleam-build :clean-tree]})]
  ...
  (testing "three-way composition: rust-build + clj-build, then explicit"
    (let [ax (config/expand-bundles {:integrity-bundle [:rust-build :clj-build]
                                    :integrity [{:op :>= :ref :disk :value 1}]})]
      (is (= 5 (count ax))
          "rust(2) + clj(2) + explicit(1)")
      (is (= :disk (-> ax last :ref)) "explicit integrity appended last"))))

(deftest expand-bundles-vector-throws-on-unknown-bundle
  (doseq [bundle [[:bogus] [:gleam-build :bogus]]]
    (is (thrown? Exception (config/expand-bundles {:integrity-bundle bundle}))
...
(deftest new-bundles-produce-build-and-test-predicates
  (letfn [(ax [b] (config/expand-bundles {:integrity-bundle b}))]
  ...
  (let [names (config-names)]
    ...
    (doseq [nm names
      ...
      (testing (str "configs/" nm ".edn validates")
        ...
        ;; clean predicates, well-formed acts/integrity checks).
...
(deftest benchmark-chase-demonstrates-vector-bundle-composition
  ...
  (let [path (str configs-dir "/benchmark-chase.edn")
        cfg  (edn/read-string (slurp path))]
    (is (vector? (:integrity-bundle cfg))
        "benchmark-chase uses a vector :integrity-bundle")
    (is (= [:rust-build :clean-tree] (:integrity-bundle cfg)))
    (is (= 3 (count (config/expand-bundles cfg)))
        "rust(2) + clean-tree(1) = 3 integrity checks after expansion")))

(defn -main
  "Run the Phase 4 bundle tests; exit non-zero on any failure or error."
  [& _]
  (let [summary (clojure.test/run-tests 'bb-agent.goal.phase4-bundles-test)]
