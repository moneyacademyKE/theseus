(ns bb-agent.goal.phase3-test
  ...
  Two Phase 3 deliverables are exercised here:
    1. Config validation at load time (reject goals without a clean
       predicate + signature, malformed integrity checks/acts).
...
(deftest validate-config-accepts-a-clean-config
  (is (empty? (config/validate-config good-cfg))
      "a well-formed config with goal + signature + act + integrity checks is valid"))
...
(deftest read-iterations-nil-or-absent-dir
  ...
  (is (empty? (log/read-iterations "/nonexistent/goal/dir" 5))
...
(defn -main
  ...
  [& _]
  (let [summary (clojure.test/run-tests 'bb-agent.goal.phase3-test)]
