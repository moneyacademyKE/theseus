(ns bb-agent.goal.phase9-opencode-dogfood-test
...
(deftest dogfood-configs-load-and-use-opencode-harness
  (doseq [f (dogfood-files)
    ...
    (testing (.getName f)
      ...
      (is (str/includes? (get-in raw [:act :prompt]) "the goal runner"))
      (is (seq (:observers raw)))
      (is (map? (:goal raw)))
      (when (or (keyword? (:integrity-bundle raw))
                (seq (:integrity-bundle raw)))
        (is (seq (:integrity loaded)) "bundle expansion keeps objective integrity checks"))
...
(deftest dogfood-playbook-documents-the-ladder
  (let [doc (slurp "docs/opencode-dogfood.md")]
    ...
    (is (str/includes? doc "the goal runner only trusts observers"))))

(defn -main
  [& _]
  (let [summary (clojure.test/run-tests 'bb-agent.goal.phase9-opencode-dogfood-test)]
