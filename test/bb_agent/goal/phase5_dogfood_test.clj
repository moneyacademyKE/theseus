(ns bb-agent.goal.phase5-dogfood-test
  ...
  (let [files (config-files)]
    ...
    (doseq [f files
      ...
      (testing (.getName f)
        ...
        (is (seq (:integrity loaded)) "bundle expansion yields executable integrity checks")))))
...
(defn -main
  [& _]
  (let [summary (clojure.test/run-tests 'bb-agent.goal.phase5-dogfood-test)]
